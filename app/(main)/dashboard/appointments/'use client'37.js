...
export default function OrderCompletePage() {
  ...
  useEffect(() => {
    if (!orderId) {
      setError(
        'No order ID found. Your order may not have been processed correctly.',
      );
      setLoading(false);
      return;
    }
  ...
  if (error) {
    return (
      <div className="text-center p-10 bg-red-50">
        ...
        <Link
          href="/"
          className="mt-6 inline-block px-6 py-2 bg-indigo-600 text-white rounded-lg"
        >
          Back to Homepage
        </Link>
      </div>
        ...
        <ul className="divide-y divide-gray-200">
          {order.order_items.map((item, index) => (
            <li key={index} className="flex items-center py-4">
              <Image
                src={item.products.image_urls?.[0] || '/placeholder-image.png'}
              ...
              <p className="font-semibold">
                R{((item.price_at_purchase * item.quantity) / 100).toFixed(2)}
              </p>
            </li>
          ))}
        </ul>
