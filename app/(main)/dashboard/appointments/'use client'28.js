    ...
    const fetchServices = async () => {
      ...
      try {
        const response = await fetch(
          `/api/public/services?salon=${salonIdentifier}`,
        );
        if (!response.ok) {
    ...
    return (
      <div className="max-w-2xl mx-auto p-8 text-center">
        ...
        <p className="text-lg text-gray-700">
          Thank you for your booking. The salon will confirm your appointment
          shortly. You will receive a confirmation via email.
        </p>
        <button
          onClick={() => router.push('/')}
        ...
        <ul className="space-y-3">
          {services.length > 0 ? (
            services.map((service) => (
              <li
                key={service.id}
                className="flex items-center justify-between p-3 border rounded-lg transition-all hover:bg-gray-50"
              >
                <label
        ...
        <div className="mt-8 bg-white p-6 rounded-lg shadow-lg sticky bottom-4 border-t-2">
          <div className="flex justify-between items-center text-lg font-medium mb-4">
            ...
            <span className="font-bold">
              {total.duration} min - R{(total.price / 100).toFixed(2)}
            </span>
          </div>
          <button
      ...
      {bookingStep === 'enter_details' && (
        <div className="mt-8 bg-white p-6 rounded-lg shadow-md">
          ...
          <div className="flex gap-4 mt-6">
            <button
              onClick={() => setBookingStep('select_services')}
            ...
            <button
              ...
              disabled={isBooking}
              className="flex-grow px-6 py-3 bg-indigo-600 text-white font-bold rounded-lg hover:bg-indigo-700 disabled:opacity-50"
            >
