...
function ServicesPageContent() {
  ...
  const addServiceMutation = useMutation({
    mutationFn: async (newService) => {
      ...
      if (!res.ok)
        throw new Error((await res.json()).error || 'Failed to add service.');
      return res.json();
    },
    onSuccess: () => {
  ...
  const updateServiceMutation = useMutation({
    mutationFn: async (updatedService) => {
      ...
      if (!res.ok)
        throw new Error(
          (await res.json()).error || 'Failed to update service.',
        );
      return res.json();
    },
    onSuccess: () => {
  ...
  const deleteServiceMutation = useMutation({
    mutationFn: async (serviceId) => {
      ...
      if (!res.ok)
        throw new Error(
          (await res.json()).error || 'Failed to delete service.',
        );
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['services'] });
  ...
  if (error) {
    return (
      <div className="p-6 max-w-4xl mx-auto text-red-600 bg-red-50 rounded-lg">
        Error: {error.message}
      </div>
    );
  }
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="bg-white p-6 rounded-lg shadow mb-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {(addServiceMutation.error || updateServiceMutation.error) && (
