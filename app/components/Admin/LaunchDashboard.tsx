...
export default function LaunchDashboard() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setMetrics(prev => ({
        ...prev,
        traffic_percent: Math.min(100, prev.traffic_percent + Math.random() * 5),
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <p className="text-3xl font-bold mt-2">{Math.round(metrics.traffic_percent)}%</p>
          <div className="w-full bg-gray-200 rounded-full h-2 mt-2">
            <div 
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <p className="text-3xl font-bold mt-2">{metrics.error_rate.toFixed(1)}%</p>
        </div>
            ...
            <div className="space-y-2">
              {metrics.regions.map(region => (
                <div key={region.name} className="flex justify-between">
                  ...
                  <span className="font-medium text-green-600">{region.status}</span>
                </div>
              ))}
            </div>
          </div>
