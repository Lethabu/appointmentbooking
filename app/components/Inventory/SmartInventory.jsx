...
const mockInventory = [
  ...
  const generateAlerts = (inventory) => {
    const alerts = [];
    ...
    inventory.forEach(item => {
      if (item.current_stock <= item.min_threshold) {
        alerts.push({
          ...
          action: item.auto_reorder ? 'Auto-reorder scheduled' : 'Manual reorder required',
          item_id: item.id
        });
...
export default function SmartInventory() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Inventory Overview */}
      <div className="bg-white rounded-lg shadow-md p-6">
        <h2 className="text-xl font-bold text-gray-900 mb-6">Smart Inventory Management</h2>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-left font-medium text-gray-700">Usage Rate</th>
                <th className="px-4 py-3 text-left font-medium text-gray-700">Predicted Stockout</th>
                <th className="px-4 py-3 text-left font-medium text-gray-700">Auto-Reorder</th>
                <th className="px-4 py-3 text-left font-medium text-gray-700">Profit Margin</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200">
              {inventory.map((item) => {
                const status = getStockStatus(item);
                const profitMargin = ((item.selling_price - item.cost_per_unit) / item.selling_price * 100).toFixed(1);
                ...
                return (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      ...
                      <div className="text-xs text-gray-500">R{item.selling_price - item.cost_per_unit} profit</div>
                    </td>
                  </tr>
                );
              })}
      ...
      {/* AI Insights */}
      <div className="bg-white rounded-lg shadow-md p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">AI Inventory Insights</h3>
        <div className="grid md:grid-cols-2 gap-6">
          <div className="space-y-4">
            <div className="p-4 bg-blue-50 rounded-lg">
              <h4 className="font-medium text-blue-900">Seasonal Prediction</h4>
              <p className="text-sm text-blue-700">Hair color demand typically increases 40% in spring. Consider increasing color stock by March.</p>
            </div>
            <div className="p-4 bg-green-50 rounded-lg">
              <h4 className="font-medium text-green-900">Cost Optimization</h4>
              <p className="text-sm text-green-700">Bulk ordering from Hair Health & Beauty could save R450/month on shampoo and treatments.</p>
            </div>
          </div>
          <div className="space-y-4">
            <div className="p-4 bg-purple-50 rounded-lg">
              <h4 className="font-medium text-purple-900">Usage Patterns</h4>
              <p className="text-sm text-purple-700">Hair extensions usage peaks on Fridays (wedding prep). Ensure stock before weekends.</p>
            </div>
            <div className="p-4 bg-yellow-50 rounded-lg">
              <h4 className="font-medium text-yellow-900">Profit Opportunities</h4>
              <p className="text-sm text-yellow-700">Retail sales of deep conditioning masks could increase revenue by R2,800/month.</p>
            </div>
          </div>
        </div>
      </div>
