  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      if (response.ok && data.authorization_url) {
        ...
        setError(
          data.error || 'Payment initialization failed. Please try again.',
        );
      }
    } catch (err) {
      console.error('Error in handleBooking:', err.message); // Debug log
      setError(
        err.message || 'An unexpected error occurred while initiating payment.',
      );
    } finally {
      setLoading(false);
        ...
        <div className="bg-gradient-to-r from-purple-600 to-pink-600 p-6 text-white">
          ...
          {servicePrice !== null && ( // Display service price if available
            <p className="text-purple-100">
              Price: R {servicePrice.toFixed(2)}
            </p>
          )}
        </div>
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              {!loading &&
                ...
                stylistId && ( // Only show slots if stylistId is available
                  <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                    {availableSlots.map(
                      (
                        time, // Use time as key
                      ) => (
                        <button
              ...
              {!loading &&
                (!availableSlots.length || !stylistId) &&
                selectedDate && ( // Show message if no slots or no stylist
                  <p className="text-gray-500 bg-gray-100 p-3 rounded-lg">
                    {stylistId
                      ? 'No available slots for this day.'
                      : 'Please select a stylist to see available times.'}
                  </p>
                )}
              {!selectedDate && (
            ...
            <div>
              <button
                ...
                disabled={
                  !selectedDate || !selectedTime || !stylistId || loading
                } // Ensure stylistId is also checked
