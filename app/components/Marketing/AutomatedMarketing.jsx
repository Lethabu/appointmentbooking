...
export default function AutomatedMarketing() {
  ...
  return (
    <div className="space-y-6">
            ...
            <div key={campaign.id} className="border border-gray-200 rounded-lg p-4">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                ...
                <div>
                  ...
                  <span className="font-semibold ml-1 text-green-600">{campaign.revenue}</span>
                </div>
              </div>
            </div>
          ))}
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Upcoming Automated Actions</h3>
        <div className="space-y-3">
          {upcomingAutomations.map((automation, index) => (
            <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              <div>
                <div className="font-medium text-gray-900">{automation.action}</div>
                <div className="text-sm text-gray-500">{automation.time}</div>
              </div>
              <div className="text-sm font-medium text-gray-700">{automation.count}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* AI Insights */}
      <div className="bg-white rounded-lg shadow-md p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">AI Marketing Insights</h3>
        <div className="space-y-4">
          <div className="p-4 bg-blue-50 rounded-lg">
            <h4 className="font-medium text-blue-900">Optimal Send Times</h4>
            <p className="text-sm text-blue-700">Your clients are most likely to open messages on Tuesday-Thursday between 2-4 PM</p>
          </div>
          <div className="p-4 bg-green-50 rounded-lg">
            <h4 className="font-medium text-green-900">High-Value Clients</h4>
            <p className="text-sm text-green-700">23 clients are due for their regular service - automated reminders scheduled</p>
          </div>
          <div className="p-4 bg-yellow-50 rounded-lg">
            <h4 className="font-medium text-yellow-900">At-Risk Clients</h4>
            <p className="text-sm text-yellow-700">15 regular clients haven't booked in 6+ weeks - win-back campaign recommended</p>
          </div>
        </div>
      </div>
    </div>
