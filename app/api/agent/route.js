  ...
  nia: [
    {
      ...
      description:
        'Get available appointment slots for a specific service and optional date.',
      parameters: {
        type: 'object',
        properties: {
          service_id: {
            type: 'string',
            description: 'The ID of the service to check for appointments.',
          },
          date: {
            type: 'string',
            description:
              'The date to check for appointments, in YYYY-MM-DD format. Defaults to today if not provided.',
          },
        },
        required: ['service_id'],
      },
      ...
      parameters: {
        ...
        properties: {
          ...
          datetime: {
            type: 'string',
            description:
              'The specific date and time for the appointment in ISO 8601 format.',
          },
          client_name: {
            type: 'string',
    ...
    },
    {
      ...
      parameters: {
        ...
        properties: {
          query: {
            type: 'string',
            description: 'The search term for services.',
          },
        },
        required: ['query'],
      },
...
async function bookAppointment(
  tenantId,
  { service_id, datetime, client_name, client_phone },
) {
...
export async function POST(req) {
  ...
  const systemMessages = {
    nia:
      `You are Nia, a friendly and efficient AI assistant${tenant ? ` for ${tenant.name}` : ''}. ` +
  ...
  try {
    ...
    if (responseMessage.function_call) {
      ...
      const functionArgsRaw = JSON.parse(
      ...
      try {
        switch (functionName) {
          case 'get_available_appointments':
            const validGetAvail =
              GetAvailableAppointmentsArgsSchema.parse(functionArgsRaw);
            functionResponse = await getAvailableAppointments(
              tenant?.id,
              validGetAvail,
            );
            break;
          ...
          case 'search_services':
            ...
            functionResponse = await searchServices(
              tenant?.id,
              validSearch.query,
            );
            break;
