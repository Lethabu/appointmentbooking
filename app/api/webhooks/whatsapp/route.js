...
export async function POST(req) {
  ...
  if (!supabaseUrl || !supabaseServiceRoleKey || !openaiApiKey) {
    console.error('Missing required environment variables for WhatsApp webhook.');
  ...
  try {
    ...
    if (!client) {
      const { data: newClient, error: insertError } = await supabase.from('clients').insert({ phone }).select('id').single();
      if (insertError) throw insertError;
      client = newClient;
    }

    // 6. Retrieve conversation history and append the new message
    const { data: conversation } = await supabase.from('conversations').select('history').eq('phone', phone).single();
    ...
    history += `\nAI: ${aiResponse}`;
    const { error: upsertError } = await supabase.from('conversations').upsert({ phone, history, client_id: client.id });
