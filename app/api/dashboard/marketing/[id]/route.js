...
export async function GET(req, { params }) {
  try {
    ...
    if (!marketing) {
      return new NextResponse(JSON.stringify({ error: 'Marketing data not found' }), { status: 404, headers: { 'Content-Type': 'application/json' } })
    }
...
export async function DELETE(req, { params }) {
  try {
    ...
    console.error('Unhandled error in DELETE /api/dashboard/marketing/[id]:', e)
    return new NextResponse(JSON.stringify({ error: 'An unexpected error occurred.' }), { status: 500, headers: { 'Content-Type': 'application/json' } })
  }
}