...
export async function POST(request) {
  try {
    ...
    if (mood_score <= 3) {
      ...
      console.log(`Crisis alert for customer ${customer_id}: mood score ${mood_score}`);
    }
    ...
    return NextResponse.json({ 
      ...
      message: discount > 0 ? `We've applied a ${discount}% mood boost discount to your next booking!` : 'Mood recorded successfully'
    });
  } catch (error) {
