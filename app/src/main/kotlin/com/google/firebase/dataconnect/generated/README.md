...
with service ID `appointmentbooking`
...
is the query named "ListServices",
which can be accessed via the [ExampleConnector.listServices] property.


An example of the property for a mutation
is the mutation named "CreateCustomer",
which can be accessed via the [ExampleConnector.createCustomer] property.
...
For example, the "ListServices" query has no variables
and can be executed via the
[ExampleConnector.listServices]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val queryResult = connector.listServices.execute()
println("ListServices query returned: ${queryResult.data}")
```
...
For example, the "CreateCustomer" mutation has no variables
and can be executed via the
[ExampleConnector.createCustomer]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val mutationResult = connector.createCustomer.execute()
println("CreateCustomer mutation returned: ${mutationResult.data}")
```
...
For example, the "UpdateAppointment" mutation has 1 required variable ("id")
and can be executed via the [ExampleConnector.updateAppointment]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val mutationResult = connector.updateAppointment.execute(id=java.util.UUID.randomUUID())
println("UpdateAppointment mutation returned: ${mutationResult.data}")
```
