...
class ApiClient {
  ...
  async getAppointments(
    ...
    let query = this.supabase.from("appointments").select("*").eq("tenant_id", tenantId)
  ...
  async createBooking(bookingData: BookingFormData & { tenantId: string }): Promise<Appointment[]> {
    ...
    const { data, error } = await this.supabase.from("appointments").insert(appointments).select()
