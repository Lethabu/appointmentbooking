...
export async function getAvailableAppointments(salonId, { service_id, date }) {
  console.log(
    `getAvailableAppointments called for salon ${salonId}, service ${service_id}, date ${date}`,
  );
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SITE_URL}/api/availability?salon_id=${salonId}&service_id=${service_id}&date=${queryDate}`,
    );
    if (!response.ok) {
...
export async function bookAppointment(
  salonId,
  { service_id, scheduled_time, client_name, client_phone, staff_id },
) {
  console.log(
    `bookAppointment called for salon ${salonId}, service ${service_id}, scheduled_time ${scheduled_time}, client ${client_name}`,
  );

  try {
    // 1. Validate availability before booking
    const availabilityResponse = await fetch(
      `${process.env.NEXT_PUBLIC_SITE_URL}/api/availability?salon_id=${salonId}&service_id=${service_id}&date=${scheduled_time.split('T')[0]}`,
    );
    ...
    const isSlotAvailable = availabilityData.available_slots.some(
      (slot) =>
        new Date(slot).toISOString() === new Date(scheduled_time).toISOString(),
    );
