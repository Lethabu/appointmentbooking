 ...
 */
export async function bookAppointment(salonId, { service_id, datetime, client_name, client_phone, staff_id }) {
  ...
  try {
    ...
    const availabilityResponse = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL}/api/availability?salon_id=${salonId}&service_id=${service_id}&date=${datetime.split('T')[0]}`);
    if (!availabilityResponse.ok) {
      const errorData = await availabilityResponse.json();
      return { error: errorData.error || 'Failed to check availability' };
    }
    const availabilityData = await availabilityResponse.json();
    const isSlotAvailable = availabilityData.available_slots.some(slot => new Date(slot).toISOString() === new Date(datetime).toISOString());
