...
-- Seed for Instyle:
WITH instyle_tenant AS (
    INSERT INTO tenants (name, subdomain, custom_domain)
...
-- Helper function to get the tenant_id for the current user
CREATE OR REPLACE FUNCTION get_user_tenant_id(user_id_param UUID)
RETURNS UUID AS $$
DECLARE
  tenant_uuid UUID;
BEGIN
  -- Attempt to get tenant_id from staff_members table first
  SELECT tenant_id INTO tenant_uuid FROM staff_members WHERE user_id = user_id_param;
  
  -- If not found in staff_members, try to get it from tenants table (for owners)
  IF NOT FOUND THEN
...
CREATE POLICY "Tenant owners can manage their own tenant details" ON tenants
  FOR ALL USING (auth.uid() = owner_id)
  WITH CHECK (auth.uid() = owner_id);

-- Policy: Appointments
CREATE POLICY "Deny access to anonymous users" ON appointments FOR ALL USING (auth.role() = 'anon');
CREATE POLICY "Allow read access to appointments for authenticated users of a specific tenant" ON appointments
  FOR SELECT USING (
    (get_user_tenant_id(auth.uid()) = tenant_id) AND
    (auth.role() = 'authenticated')
  );
CREATE POLICY "Allow full access to appointments for tenant staff" ON appointments
  FOR ALL USING (
    (get_user_tenant_id(auth.uid()) = tenant_id) AND
    (get_user_role(tenant_id, auth.uid()) IN ('owner', 'manager', 'staff'))
  )
  WITH CHECK (
    (get_user_tenant_id(auth.uid()) = tenant_id) AND
    (get_user_role(tenant_id, auth.uid()) IN ('owner', 'manager', 'staff'))
...
-- Policy: Services
CREATE POLICY "Deny access to anonymous users" ON services FOR ALL USING (auth.role() = 'anon');
CREATE POLICY "Allow read access to services for authenticated users of a specific tenant" ON services
  FOR SELECT USING (
    (get_user_tenant_id(auth.uid()) = tenant_id) AND
    (auth.role() = 'authenticated')
  );
CREATE POLICY "Allow full access to services for tenant staff" ON services
  FOR ALL USING (
    (get_user_tenant_id(auth.uid()) = tenant_id) AND
    (get_user_role(tenant_id, auth.uid()) IN ('owner', 'manager', 'staff'))
  )
...
-- Policy: Products
CREATE POLICY "Deny access to anonymous users" ON products FOR ALL USING (auth.role() = 'anon');
CREATE POLICY "Allow read access to products for authenticated users of a specific tenant" ON products
  FOR SELECT USING (
    (get_user_tenant_id(auth.uid()) = tenant_id) AND
    (auth.role() = 'authenticated')
  );
CREATE POLICY "Allow full access to products for tenant staff" ON products
  FOR ALL USING (
    (get_user_tenant_id(auth.uid()) = tenant_id) AND
    (get_user_role(tenant_id, auth.uid()) IN ('owner', 'manager', 'staff'))
  )
...
-- Policy: Orders
CREATE POLICY "Allow read access to orders for authenticated users of a specific tenant" ON orders
  FOR SELECT USING (
    (get_user_tenant_id(auth.uid()) = tenant_id) AND
    (auth.role() = 'authenticated')
  );
CREATE POLICY "Allow full access to orders for tenant staff" ON orders
  FOR ALL USING (
    (get_user_tenant_id(auth.uid()) = tenant_id) AND
    (get_user_role(tenant_id, auth.uid()) IN ('owner', 'manager', 'staff'))
  )
...
CREATE OR REPLACE FUNCTION get_user_role(p_tenant_id uuid, p_user_id uuid)
RETURNS TEXT AS $$
DECLARE
  user_role TEXT;
BEGIN
  -- Check if owner first
  PERFORM 1 FROM tenants WHERE id = p_tenant_id AND owner_id = p_user_id;
  IF FOUND THEN
    RETURN 'owner';
  END IF;
...
-- Function for global search within a tenant
...
BEGIN
  RETURN QUERY
    ...
    WHERE a.tenant_id = p_tenant_id AND (p.full_name ILIKE '%' || search_term || '%' OR s.name ILIKE '%' || search_term || '%')
    UNION ALL
    -- Clients
    SELECT pr.id, 'client' AS type, pr.full_name AS title,
           CONCAT('Phone: ', COALESCE(pr.phone, 'N/A')) AS subtitle
    FROM profiles pr
    WHERE pr.tenant_id = p_tenant_id AND (pr.full_name ILIKE '%' || search_term || '%' OR pr.phone ILIKE '%' || search_term || '%')
    UNION ALL
    -- Products
    ...
    FROM products prod
    WHERE prod.tenant_id = p_tenant_id AND prod.name ILIKE '%' || search_term || '%'
    LIMIT 10;
END;
$$ LANGUAGE plpgsql;

-- ... other advanced functions like get_platform_stats, get_tenant_financials, etc. would go here ...
...
CREATE INDEX idx_staff_members_tenant_user ON staff_members (tenant_id, user_id);

-- Supabase RPC for get_dashboard_stats
CREATE OR REPLACE FUNCTION get_dashboard_stats(tenant_id_param uuid)
...
BEGIN
  RETURN QUERY
  SELECT
    (SELECT COUNT(*) FROM appointments WHERE tenant_id = tenant_id_param) as total_appointments,
    (SELECT SUM(price) FROM services s JOIN appointments a ON s.id = a.service_id WHERE a.tenant_id = tenant_id_param) as total_revenue,
    (SELECT MAX(created_at) FROM appointments WHERE tenant_id = tenant_id_param) as latest_booking;
END;
$$;
