  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const { data: servicesData } = await supabase
        .from('services')
        .select(
          'id, name, description, price_cents, duration_minutes, category_id, service_categories(name)',
        )
        .eq('salon_id', process.env.NEXT_PUBLIC_INSTYLE_SALON_ID) // Use NEXT_PUBLIC for client-side env vars
        .order('sort_order', {
          referencedTable: 'service_categories',
          ascending: true,
        });
  ...
  async function handleBookingSubmit(e) {
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        setBookingMessage(
          'Booking requested successfully! We will confirm shortly.',
        );
        // Clear form
        setFullName('');
        setEmail('');
        ...
        setBookingStatus('error');
        setBookingMessage(
          response.data.message || 'Failed to request booking.',
        );
      }
    } catch (error) {
      console.error(
    ...
    <div className="bg-white rounded-xl shadow p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div className="flex items-center gap-4 flex-1">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">
            Booked by:{' '}
            {appointment.users?.name || appointment.users?.email || 'N/A'}
          </p>
        </div>
      </div>
