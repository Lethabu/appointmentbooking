...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!supersaasResponse.ok) {
      ...
      return res.status(500).json({ message: 'Error fetching bookings from SuperSaaS', error: errorData });
    }

    const { bookings: supersaasBookings } = await supersaasResponse.json();

    if (!supersaasBookings || supersaasBookings.length === 0) {
      return res.status(200).json({ message: 'No new bookings from SuperSaaS to sync.' });
    }
    ...
    if (existingError) {
      ...
      return res.status(500).json({ message: 'Error fetching existing appointments from Supabase', error: existingError });
    }

    const existingSupersaasIds = new Set(existingAppointments.map((a) => a.supersaas_booking_id));

    const bookingsToInsert = supersaasBookings.filter(
      (b) => !existingSupersaasIds.has(b.id)
    ).map((b) => ({
    ...
    if (bookingsToInsert.length > 0) {
      ...
      if (insertError) {
        ...
        return res.status(500).json({ message: 'Error inserting new bookings into Supabase', error: insertError });
      }
      return res.status(200).json({ message: `Successfully synced ${insertedBookings.length} bookings.`, insertedBookings });
    } else {
      return res.status(200).json({ message: 'No new bookings to insert into Supabase.' });
    }
  } catch (error) {
